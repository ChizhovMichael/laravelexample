...
<body>
    <div class="modal__wrapp col-6 sd-12 shadow-xs back-body b8 hide">
        ...
        <h5 class="text-center">Подтверждение отправки!</h5>
        <div class="pl-em-3 pr-em-3 pb-em-3">
            <p class="cc">Уважаемый, {{ $contact['order_lname'] }} {{ $contact['order_fname'] }} {{ $contact['order_mname'] }}.</p>
            <p class="cc">Уведомляем вас о том, что мы отправили ваш заказ <span class="bold ct">#{{ $contact['id'] }}</span> по вашем адресу: {{ $contact['order_city'] }} {{ $contact['order_address'] }}</p>
            <p class="cc">Ваш трек номер: <span class="bold ct">{{ $contact['order_tracking'] }}</span></p>
